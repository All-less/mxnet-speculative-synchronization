...
void Van::OnRecvNotif(Message &msg) {
  ...
  if (push_index_ == num_workers_) {
    ...
    auto cur_wait_time_ = TO_MS(wait_ratio_ * epoch_length_);
    epoch_index_ += 1;
...
void Van::InitializeCancellation() {
  push_index_ = 0;
  epoch_index_ = 0;
  wait_ratio_ = std::atof(Environment::Get()->find("MXNET_WAIT_RATIO"));
  cancel_threshold_ = std::atof(Environment::Get()->find("MXNET_CANCEL_THRESHOLD"));
...
void Van::CheckCancellation(int id, std::chrono::system_clock::time_point &now) {
  ...
  auto threshold = cancel_threshold_;
