...
class Van {
 ...
 private:
  ...
  /** find optimal wait time according to predicted trace */
  void PredictWaitTime(int id);
  /** Find optimal wait time based on current push_trace_ */
  std::chrono::milliseconds FindWaitTime();
  /** update cancellation-related information */
  void OnRecvNotif(Message &msg);
  /** update prediction of the trace */
  void UpdatePrediction(int id, std::chrono::system_clock::time_point next, double var);
  ...
  /** number of epochs */
  int epoch_index_;
  /** period length of history waiting time */
  std::unique_ptr<LengthList> wait_time_history_;
