...
#define MS_TO_DOUBLE(x) std::chrono::duration_cast<std::chrono::milliseconds>(x).count()*1.0
...
void Van::OnRecvNotif(Message &msg) {
  ...
  // LOG(INFO) << "Received notification message. push_index_ is " << push_index_;

  if (push_index_ == num_workers_) {
    // find optimal wait_time
    epoch_length_ = std::chrono::duration_cast<std::chrono::milliseconds>(time - push_trace_->at(0));
    auto cur_wait_time_ = FindWaitTime();
    epoch_index_ += 1;
    wait_time_ = (wait_time_ * (epoch_index_ - 1) + cur_wait_time_) / epoch_index_;
    LOG(INFO) << "wait_time for next epoch is " << MS_TO_DOUBLE(wait_time_);

    // update push trace
    push_trace_->at(0) = time;
    push_index_ = 1;
  } else {
    push_trace_->at(push_index_) = time;
    push_index_ += 1;
  }
  ...
  /*
  if (epoch_count_ == nullptr) {
    InitializeCancellation();
  }
  int id = Postoffice::IDtoRank(msg.meta.sender);
  auto time = std::chrono::system_clock::now();
  auto num = epoch_count_->at(id);

  if (num == 0) {
    // InitializeCancellation();
  } else if (num > 0) {
    // calculate average length and variance
    std::chrono::milliseconds avg_len = epoch_length_->at(id);
    std::chrono::milliseconds cur_len = std::chrono::duration_cast<std::chrono::milliseconds>(time - epoch_start_->at(id));
    std::chrono::milliseconds new_len = std::chrono::duration_cast<std::chrono::milliseconds>((avg_len * (num - 1) + cur_len) / num);
    auto avg_len_d = MS_TO_DOUBLE(avg_len);
    auto cur_len_d = MS_TO_DOUBLE(cur_len);
    auto new_len_d = MS_TO_DOUBLE(new_len);
    auto var = epoch_variance_->at(id);
    auto new_var = 0; // (var * (num - 2) + (cur_len_d - new_len_d) * (cur_len_d - avg_len_d)) / (num - 1);
    epoch_length_->at(id) = new_len;
    epoch_variance_->at(id) = new_var;

    // LOG(INFO) << "Worker: " << id << " Count: " << num << " Length: " << new_len_d << " Average: " << avg_len_d;

    // insert predicted arrival
    UpdatePrediction(id, time + avg_len, new_var);

    // insert CHECK and PREDICT tasks
    waiting_workers_->insert(id);
    if (MS_TO_DOUBLE(wait_time_->at(id)) > 0) {
      task_queue_->push({ CHECK, id, time + wait_time_->at(id) });
    }
    task_queue_->push({ PREDICT, id, time + avg_len / 3 });
  ...
  epoch_count_->at(id) += 1;
  epoch_start_->at(id) = time;
  */
}

std::chrono::milliseconds Van::FindWaitTime() {
  double epoch_length = MS_TO_DOUBLE(epoch_length_);
  std::priority_queue<double> dist;
  for (int i = 0; i < num_workers_; ++i) {
    for (int j = i + 1; j < num_workers_; ++j) {
      dist.push(MS_TO_DOUBLE(push_trace_->at(j) - push_trace_->at(i)));
    }
  }
  double wait, gain, max_wait = 0, max_gain = std::numeric_limits<double>::min();
  while (!dist.empty()) {
    wait = dist.top();
    dist.pop();
    gain = dist.size() - num_workers_ * (num_workers_ - 1) * wait / epoch_length / 4.0;
    if (gain > max_gain && wait > epoch_length * 0.05) {
      max_wait = wait;
      max_gain = gain;
    }
  }

  return std::chrono::milliseconds(int(max_wait == 0 ? epoch_length / 10 : max_wait));
}

void Van::InitializeCancellation() {
  push_index_ = 0;
  epoch_index_ = 0;
  wait_time_history_ = std::unique_ptr<LengthList>(
    new LengthList(5)
  );
...
void Van::UpdatePrediction(int id, std::chrono::system_clock::time_point next, double var) {
  /*
  auto prev_pred = std::find_if(
    pred_trace_->begin(),
    pred_trace_->end(),
    [id](PredTime &t) { return t.id == id; }
  );
  if (prev_pred != pred_trace_->end()) {
    pred_trace_->erase(prev_pred);
  }
  auto pos = std::find_if(
    pred_trace_->rbegin(),
    pred_trace_->rend(),
    [next](PredTime &t) { return t.time < next; }
  );
  pred_trace_->insert(pos.base(), { id, next, var });
  */
  /*
  std::stringstream ss;
  ss << "[ ";
  for (auto p : *pred_trace_) {
    ss << "( " << p.id << " " << std::chrono::duration_cast<std::chrono::milliseconds>(p.time.time_since_epoch()).count() << " ) ";
  }
  ss << "]";
  LOG(INFO) << ss.str();
  */
...
void Van::CheckCancellation(int id, std::chrono::system_clock::time_point &now) {
  ...
  auto threshold = MS_TO_DOUBLE(wait_time_) / MS_TO_DOUBLE(epoch_length_) * (num_workers_ - 1) / 4.0;
  LOG(INFO) << "Worker " << id << " got " << updates << " updates, expected " << threshold << ".";
    ...
    */
    ...
    if (!msg.meta.control.empty()) {
      ...
      if (ctrl.cmd == Control::TERMINATE) {
        ...
        // The receiver of NOTIFY message should be a scheduler.
        if (is_scheduler_) {
          OnRecvNotif(msg);
        }
