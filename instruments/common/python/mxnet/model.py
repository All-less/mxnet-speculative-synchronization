...
DELAY = int(os.getenv('MXNET_DELAY') or 0)
...
def _update_params_on_kvstore(param_arrays, grad_arrays, kvstore, log_callback=None, pull_only=False):
    """Perform update of param_arrays from grad_arrays on kvstore."""
    def log(content):
        if log_callback:
            log_callback(content)
    length = len(param_arrays)
    if not pull_only:
        for index, pair in enumerate(zip(param_arrays, grad_arrays)):
            ...
            kvstore.push(index, grad_list, priority=-index, last=(1 if index == length - 1 else 0))
        log('PUSHED')
    time.sleep(DELAY)
    log('PULL')
    for index, pair in enumerate(zip(param_arrays, grad_arrays)):
