...
class ThreadedEnginePerDevice : public ThreadedEngine {
 public:
  ...
  void ClearTask() override {
    LOG(WARNING) << "Engine::ClearTask is not implemented in ThreadedEnginePerDevice.";
    /*
    if (cpu_worker_nthreads_ + gpu_worker_nthreads_ + gpu_copy_nthreads_ > 32) {
      LOG(WARN) << "ThreadedEnginePerDevice::ClearTask does not support over 32 threads yet";
    }
    */
  }
