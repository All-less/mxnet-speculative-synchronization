...
void ThreadedVar::ClearDependencies() {
  if (clearing_dependencies_.load()) {
    return;
  }
  std::lock_guard<std::mutex> lock{m_};
  clearing_dependencies_.store(true);
  while (pending_write_ && pending_write_ != head_) {
    pending_write_->trigger->ClearDependencies();
    pending_write_ = pending_write_->next;
  }
  pending_write_ = nullptr;
  num_pending_reads_ = 0;
  // TODO: check whether to_delete is set
  clearing_dependencies_.store(false);
}
...
inline void ThreadedVar::CompleteReadDependency(Dispatcher dispatcher) {
  OprBlock *trigger = nullptr;
  {
    ...
    std::lock_guard<std::mutex> lock{m_};
    if (num_pending_reads_ <= 0) {
      // the violation means we just called ClearDependencies,
      // so the dispatcher should be discarded
      return;
    }
...
inline bool ThreadedVar::CompleteWriteDependency(Dispatcher dispatcher) {
  ...
  {
    std::lock_guard<std::mutex> lock{m_};
    if (num_pending_reads_ != kWriteTriggered) {
      // the violation means we just called ClearDependencies,
      // so the dispatcher should be discarded
      return false;
    }
...
void OprBlock::ClearDependencies() {
  if (!clearing_dependencies_.load()) {
    clearing_dependencies_.store(true);
    if (opr) {
