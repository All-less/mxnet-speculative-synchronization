...
struct OprBlock : public common::ObjectPoolAllocatable<OprBlock> {
  ...
  /*! \brief A flag for loop detection in ClearDependencies */
  std::atomic<bool> clearing_dependencies_{false};
  ...
  /*!
   * \brief Clear all dependencies related to this operator.
   */
...
class ThreadedVar final : public Var,
 ...
 public:
  ...
  /*! \brief Clear all read/write dependencies. */
  void ClearDependencies();
...
// private:
  ...
  std::mutex m_;
  /*! \brief A flag for loop detection */
  std::atomic<bool> clearing_dependencies_{false};
  /*!
...
class ThreadedEngine : public Engine {
 ...
 protected:
  ...
  /*!
   * \brief a flag to synchronize termination among threads
   */
