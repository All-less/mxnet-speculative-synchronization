...
class KVWorker : public SimpleApp {
 public:
  ...
  /**
   * \brief report progress ratio to scheduler
   */
  void ReportLoss(double loss) {
    // convert ratio to string representation
    std::stringstream ss;
    ss << loss;
    Message msg;
    msg.meta.control.cmd = Control::REPORT;
    msg.meta.body = ss.str();
    msg.meta.customer_id = obj_->id();
    msg.meta.request = false;
    msg.meta.push = false;
    msg.meta.recver = Postoffice::Get()->GetNodeIDs(kScheduler).at(0);
