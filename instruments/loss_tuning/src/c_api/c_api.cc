...
int MXKVStorePush(KVStoreHandle handle,
  ...
  static_cast<KVStore*>(handle)->Push(v_keys, v_vals, priority, last);
  API_END();
}
...
int MXKVStoreSetCancelCallback(KVStoreHandle handle,
                               CancelCallback callback) {
  API_BEGIN();
  static_cast<KVStore*>(handle)->SetCancelCallback(callback);
  API_END();
}
...
MXNET_DLL int MXKVStoreReportLoss(KVStoreHandle handle,
                                   double loss) {
  API_BEGIN();
  static_cast<KVStore*>(handle)->ReportLoss(loss);
  API_END();
}
