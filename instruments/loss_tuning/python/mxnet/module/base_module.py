...
class BaseModule(object):
    ...
    def fit(self, train_data, eval_data=None, eval_metric='acc',
            ...
            validation_metric=None, monitor=None, report_metrics=[]):
        ...
        for epoch in range(begin_epoch, num_epoch):
            ...
            while not end_of_batch:
                ...
                self._batch_length = (1.0 * self._batch_length * nbatch + (end - start)) / (nbatch + 1)
                ...
                if self._enable_cancel and len(report_metrics) > 0:
                    for name, val in eval_metric.get_name_value():
                        if name in report_metrics:
                            self.logger.info('Report loss {} to scheduler.'.format(val))
                            kvstore.report_loss(val)
