...
std::chrono::milliseconds Van::FindWaitTime() {
  ...
  double wait, gain;
  double max_wait = max_wait_ratio_ * epoch_length / 3;
  double max_gain = std::numeric_limits<double>::min();
  while (!dist.empty()) {
    wait = dist.top();
    dist.pop();
    gain = dist.size() - num_workers_ * (num_workers_ - 1) * wait / epoch_length / 4.0;
    if (gain > max_gain && wait > epoch_length * 0.03) {
      max_wait = wait;
      max_gain = gain;
    }
  }

  return std::chrono::milliseconds(int(std::min(max_wait, max_wait_ratio_ * epoch_length)));
}

void Van::InitializeCancellation() {
  push_index_ = 0;
  epoch_index_ = 0;
  max_wait_ratio_ = std::sqrt(2 * (num_workers_ * num_workers_ + 1)) / (num_workers_ + 1) - 1;
